...
export class SharedService {
  ...
  ageToString(age: number) {
    ...
    if (minutes !== 0) {
      ...
      result += minutes === 1 ? this.translate.instant('MINUTE') : this.translate.instant('MINUTES');
    }
    if (seconds !== 0) {
      if (result !== '') { result += ', '; }
      result += String(seconds) + ' ';
      result += seconds === 1 ? this.translate.instant('SECOND') : this.translate.instant('SECONDS');
    }
    return result;
  }
  ...
  is_knx_groupaddress(groupaddress: string) {
    ...
    if (!(this.isInt(g[0]) && this.isInt(g[1]) && this.isInt(g[2]))) { return false; }
    if ((Number(g[0]) < 0) || (Number(g[0]) > 31)) { return false; }
    if ((Number(g[1]) < 0) || (Number(g[1]) > 7)) { return false; }
    if ((Number(g[2]) < 0) || (Number(g[2]) > 255)) { return false; }
    return true;
  }
  ...
  is_ipv6(value: string) {
    ...
    if (components.length < 2 || components.length > 8) { return false; }
    if (components[0] !== '' || components[1] !== '') {
      if (!components[0].match(/^[\da-f]{1,4}/i)) { return false; }
    }
  ...
  getBrowser() {
    ...
    if (vtem != null) { M.splice(1, 1, vtem[1]); }
  ...
  setGuiLanguage() {
    ...
    if (installed_languages.indexOf(lang) > -1) {
      ...
      console.warn('SharedService.setGuiLanguage', 'language ' + lang + ' not installed, using en instead');
  ...
  getFallbackLanguage(index: number = 0): string {
    ...
    if (!order || order.length === 0) {
      console.warn('SharedService.getFallbackLanguage: fallbackLanguageOrder is empty, using defaults');
